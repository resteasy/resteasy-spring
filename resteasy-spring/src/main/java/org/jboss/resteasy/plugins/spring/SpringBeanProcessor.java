...
public class SpringBeanProcessor implements BeanFactoryPostProcessor, SmartApplicationListener {
    ...
    private static Class<?> getBeanClass(String name, BeanDefinition beanDef,
        ...
        if (factoryMethodName != null) {
             ...
             * 2. Bean B is retrieved by telling Spring that the Factory-Bean is Bean A and that there is a method X to retrieve
             * Bean B.
             * example: <bean id="repositoryService" factory-bean="processEngine" factory-method="getRepositoryService"/>
             * 3. When resteasy has to inject Bean B it tries to lookup method X on Bean A instead of Bean B using the above
             * code.
             *
             * As a fix for this, we retrieve the return type for Bean A from the FactoryBean, which later on can be used to
             * retrieve the other beans.
             *
             */
